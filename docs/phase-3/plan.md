...
| 구분 | 수량 | 상태 |
|------|------|------|
| 발행 완료 | 7개 | `<!-- URL-NEEDED -->` 3개 글에 잔존 |
| 초안 대기 | 7개 | 운영자 승인 후 발행 가능 |

**잔여 작업:**
- [ ] `baby-items-cost.md` URL-NEEDED 링크 삽입
- [ ] `hospital-bag.md` URL-NEEDED 링크 삽입
- [ ] `postpartum-care.md` URL-NEEDED 링크 삽입
- [ ] PERSONAL EXPERIENCE 실제 경험담 교체 (잔여 2개 글)
- [ ] `infant-vaccination-schedule.md` 1,000단어+ 보강 (현재 611단어)
- [ ] 초안 7개 중 우선 3~4개 발행 (5/1 전 목표)

### 베이비페어 (babyfair_events.json)

| 항목 | 현황 |
|------|------|
| 등록 이벤트 | 31건 (2026-01~12) |
| 지난 이벤트 처리 | ✅ 날짜 경과 시 "지난행사" 카테고리로 자동 분류 |

**잔여 작업:**

- [ ] 2026 하반기 신규 베이비페어 정보 추가 확인

### 타임라인 (timeline_items.json)

| 항목 | 현황 |
|------|------|
| 전체 항목 | ~35개 |
| 영상 매핑 완료 | 8개 주차 (12, 21, 22, 25, 30, 31, 32, 34주) |
| 영상 매핑 없음 | ~27개 주차 |

**잔여 작업:**
- [ ] 핵심 주차 영상 매핑 확대 (초기 4~8주, 출산 직전 36~40주 우선)
- [ ] Phase-4 크로스링크 자동화 전까지 수동 매핑 권장
...
**잔여 작업:**
- [ ] 신규 18개 채널 썸네일 수집 (`npm run fetch-channel-thumbs`)
- [ ] 보롯TV youtube_channel_id 수동 확인 (검색 미발견)

### About 페이지

- [ ] 운영자 프로필 정보 보강 (필명, 프로필 이미지)
- [ ] E-E-A-T 심사 대비 전문성/경험 어필 텍스트 추가
