...
Phase 4.6에서 4축(체크리스트·베이비페어·블로그·체중)으로 정보 구조를 좁히고 AdSense 6월 신청 통과 상태로 산후 휴면에 들어간다. Phase 5는 **산후 복귀(2026-11~) 후 4축 fit이 검증된 자산에 도구·콘텐츠를 덧대는 단계**.

운영 무게는 [persona.md §3.3](../content/persona.md)대로 "신청 통과 가능성"에서 **"정책 위반 회피 + 사용자 가치"** 로 이동.
...
> 본 문서는 F1만 상세 설계. F2~F6은 후보 등재까지만. Phase 5 진입 라운드에서 운영자 우선순위 결정 라운드를 따로 잡는다.
