...
| 1.5 | 로컬 | 타임라인 + 체크리스트 통합 | 1~2주 | 📋 기획 완료 |
| 1.75 | 로컬 → 커스텀 도메인 | Google AdSense 승인 준비 | 1주 + 심사 대기 | 📋 기획 완료 |
...
- **정보 분절**: "28주에 뭘 준비해야 하지?"에 답을 얻으려면 2개 페이지를 오가야 함
...
- 하단 네비: 5탭 재구성 (홈/타임라인/베이비페어/체중/영상) — 홈 Feature Grid과 일치
...
- "이번 주 할 일"에 커스텀 타임라인 항목(`useTimelineStore.customItems`)도 포함
- 대시보드 하단 **피드백 배너** 추가 (구글 폼 링크, 새 탭 열기)
  - 환경변수: `NEXT_PUBLIC_FEEDBACK_FORM_URL` (값 미설정 시 배너 미노출)

### 1.5-8. 베이비페어 외부 링크 팝업

- 베이비페어 카드 클릭 시 외부 이동 확인 팝업 (AlertDialog)
- 확인 시 `official_url`을 새 탭으로 열기
- `official_url` 미존재 시 클릭 비활성

---

## Phase 1.75. Google AdSense 승인 준비 (로컬 → 커스텀 도메인)

> 상세 스펙: [phase-1.75/plan.md](../phase-1.75/plan.md)

Phase 1.5 완료 후, AdSense 승인을 받기 위한 **SEO 기반 작업 + 콘텐츠 보완 + 신뢰성 페이지**를 추가한다.
PoC 기간(4주) 내에 승인 신청 → 트래픽 + 수익 동시 검증이 목표.

### 1.75-1. 커스텀 도메인 연결 (필수)

- `github.io`는 공유 도메인이라 AdSense 승인 불가
- 도메인 구매 → GitHub Pages 커스텀 도메인 설정 + HTTPS 활성화
- `public/CNAME` 파일 추가, `basePath` 제거

### 1.75-2. sitemap.ts + robots.ts

- `src/app/sitemap.ts` — 전체 페이지 URL 포함 동적 사이트맵
- `src/app/robots.ts` — 크롤링 허용 + sitemap URL 명시
- Google Search Console에 사이트 등록 + sitemap 제출

### 1.75-3. About 페이지

- 서비스 소개, 타겟 사용자, 제공 기능 요약, 연락처
- 푸터에 About 링크 추가
- AdSense 리뷰어가 운영자 신뢰성 확인에 사용

### 1.75-4. 페이지별 메타데이터

- 각 페이지에 고유한 `title` + `description` export
- Google 색인 품질 향상 + 검색 결과 노출 개선

### 1.75-5. 영상 데이터 확보

- `videos.json`이 빈 배열 → 빈 페이지는 "저품질 콘텐츠" 거절 사유
- 최소 6~9개 영상 수동 큐레이션 (카테고리별 2~3개)
- 또는 데이터 확보 전까지 네비에서 임시 제거

### 1.75-6. 가이드 콘텐츠 (권장)

- 구조화 데이터만으로는 콘텐츠 깊이 부족
- "출산 가방 필수 준비물 가이드", "임신 주차별 검사 가이드" 등 1~2개 작성
- 기존 데이터를 설명형 글로 재구성 → SEO 랜딩 효과

### 1.75-7. AdSense 승인 신청

- Step 1~5 완료 + 2~4주 운영 후 신청
- 심사 1~4주 소요, 거절 시 보완 후 재신청 가능
