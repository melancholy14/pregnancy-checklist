...
## Phase 3. 누락 기능 보완 + AdSense 승인 준비 (로컬)

> Phase 2.5까지 누락된 기능을 보완하고, AdSense 승인에 필요한 인프라를 갖춘다.
> 체크리스트/도구 중심 SaaS로서 승인을 목표로 하며, 블로그 콘텐츠는 2~3일 간격으로 병렬 보강한다.
> 목표: ~2026-05-01

### AdSense 인프라 (선행)

#### 3-0a. AdSense 스크립트 로드 + AdUnit 배치 + ads.txt

AdSense 심사를 위한 광고 인프라를 구성한다.

- `layout.tsx`의 `<head>`에 `adsbygoogle.js` 스크립트 태그 추가 (현재 `<meta>` 태그만 존재)
- `public/ads.txt` 생성 (`google.com, pub-XXXXXXXXXXXXXXXX, DIRECT, f08c47fec0942fa0`)
- `AdUnit` 컴포넌트를 최소 3곳에 배치:
  - 아티클 상세 페이지 (본문 중간 + 하단)
  - 홈 피드
  - 타임라인 페이지
- 광고 슬롯은 콘텐츠 몰입을 방해하지 않는 자연스러운 위치에 배치

**완료 조건**: 배포 후 AdSense 콘솔에서 사이트 크롤링 가능 상태 확인, ads.txt 접근 가능

#### 3-0b. 쿠키 동의 배너 구현

개인정보처리방침에 명시된 "첫 방문 시 쿠키 안내 제공"을 실제로 구현한다.

- `CookieConsentBanner` 컴포넌트 작성
- localStorage 기반 동의 상태 관리 (동의 후 배너 미노출)
- 하단 고정 배너 UI: GA4/AdSense 쿠키 사용 안내 + 동의/거부 버튼
- 거부 시 GA4·AdSense 스크립트 비활성화 처리

**완료 조건**: 첫 방문 시 배너 노출, 동의/거부 동작 확인, 개인정보처리방침 내용과 실제 동작 일치

#### 3-0c. canonical URL 플레이스홀더 수정

`{{SITE_URL}}` 미치환 문제를 해결한다.

- `src/content/articles/*.md` 전체 스캔하여 `{{SITE_URL}}` 플레이스홀더 확인
- 실제 도메인(`https://pregnancy-checklist.com`)으로 일괄 치환
- 빌드 타임에 자동 치환하는 방식도 검토 (향후 아티클 추가 시 실수 방지)

**완료 조건**: 모든 아티클의 canonical URL이 정상 도메인으로 출력

### 도구 페이지 가치 강화 (SaaS 핵심)

#### 3-0d. 도구 페이지별 설명 텍스트 추가

AdSense 심사 봇이 텍스트 콘텐츠를 파싱하므로, 각 도구 페이지에 "왜 필요한지, 어떻게 활용하는지" 설명을 추가한다.

| 페이지 | 추가 내용 | 분량 |
|--------|----------|------|
| 체크리스트 | 주차별 체크리스트가 필요한 이유, 활용 가이드 | 200~300자 |
| 타임라인 | 임신 주차별 준비 흐름을 한눈에 보는 방법 | 200~300자 |
| 체중 관리 | 적정 체중 증가 범위와 기록의 중요성 | 200~300자 |
| 베이비페어 | 전국 행사 일정을 놓치지 않는 방법 | 200~300자 |
| 영상 | 검증된 채널의 큐레이션 영상 활용법 | 200~300자 |

**완료 조건**: 5개 도구 페이지 모두 설명 텍스트가 렌더링되고, 심사 봇이 텍스트를 크롤링 가능

#### 3-0e. reviewed_by 필드 정리 + 의료 디스클레이머 추가

YMYL 카테고리 대응을 위한 신뢰도 보완.

- 9개 아티클의 `reviewed_by: ""` 빈 필드를 **제거** (빈 값 노출은 "리뷰받지 않았다"를 명시하는 것과 동일)
- 아티클 상세 페이지 상단에 의료 디스클레이머 추가: "본 글은 일반 정보 제공 목적이며 의학적 조언을 대체하지 않습니다. 정확한 진단과 치료는 전문의와 상담하세요."
- 디스클레이머는 공통 컴포넌트로 작성하여 모든 아티클에 일괄 적용

**완료 조건**: reviewed_by 빈 필드 제거, 모든 아티클 상단에 디스클레이머 노출
...
> 아래 항목은 개발 작업과 병렬로 운영자가 진행한다. 2~3일 간격으로 콘텐츠를 추가/보강.

- 기존 9개 아티클의 `<!-- URL-NEEDED -->` 참고자료 링크 전수 삽입 (~30개)
- 기존 9개 아티클의 `<!-- PERSONAL EXPERIENCE -->` 플레이스홀더를 실제 경험담으로 교체
- 신규 아티클 추가 (2~3일 간격, 최소 1,000단어 이상)
- About 페이지 운영자 정보 보강 (필명, 프로필 이미지)
...
### 4-1. 체중 기록 페이지 차트 시각화 강화

임신 전 키/체중 입력 → BMI 자동 계산 → IOM 2009 기준 권장 범위 영역 그래프 표시.
ComposedChart(Line + Area), 커스텀 Tooltip, 요약 통계 카드 추가.

- 선행 조건: 의료 면책 프레임워크 확립 + 개인정보처리방침 건강 정보 조항 추가
- 법적 리스크 HIGH: YMYL 의료정보, 개인 건강정보(localStorage), 의료기기법 저촉 주의

**완료 조건**: BMI별 권장 범위 영역 표시, 의료 면책 문구 차트 하단 직접 배치, 미입력 시 기존 차트 하위 호환

### 4-2. 아티클 하단 관련 콘텐츠 추천

아티클 상세 하단에 태그 Jaccard 유사도 기반 관련 글 3개 추천.
태그 겹침 1개+ 우선, 부족 시 최신 글 보충. Phase 4.2에서 타임라인/영상 크로스 추천 확장.

**완료 조건**: 관련 글 최대 3개 표시, 현재 글 제외, 클릭 시 정상 이동

### 4-3. 공유 기능

Web Share API(모바일 네이티브 시트) → Clipboard API(링크 복사) → 카카오 SDK(데스크톱 fallback) 단계 구현.
OG 메타 태그 정비 + GA4 `share` 이벤트 전송.

**완료 조건**: 모바일 네이티브 공유 시트, 데스크톱 링크 복사 + 토스트, OG 프리뷰 정상

### 4-4. 크로스링크 영역 확장

영상→타임라인(역방향), 영상↔블로그, 베이비페어↔타임라인/블로그 연결.
`videos.json`, `babyfair_events.json`, 아티클 front matter에 매핑 필드 추가.

**완료 조건**: 영상/베이비페어 카드에서 관련 콘텐츠 링크 노출, 종료 이벤트 링크 비활성화

### 4-5. 크로스링크 자동 생성 스크립트

`scripts/generate-crosslinks.ts` — 태그 Jaccard + 키워드 매칭으로 크로스링크 자동 생성.
`--dry-run` / `--apply` / `--report` 모드. `*_manual: true` 수동 매핑 보호.

**완료 조건**: 드라이런 정상 출력, apply 시 파일 갱신, manual 매핑 보호, 빌드 성공
