...
> Date: 2026-04-02
> Updated: 2026-04-03 (기획 검증 후 보완 사항 반영)
> Status: 📋 기획 완료
...
| **네비게이션** | 5개 탭 (홈/체크리스트/타임라인/체중/더보기) | 5개 탭 (홈/타임라인/베이비페어/체중/영상) — 홈 Feature Grid과 동일 구성 |
...
- 카테고리 필터 (보조 뷰)
- "행정 준비" 카테고리에 지자체 확인 안내 문구 표시
- 커스텀 항목 삭제 시 확인 다이얼로그
- 대시보드 "이번 주 할 일"에 커스텀 타임라인 항목 포함
- `/checklist` → `/timeline` 리다이렉트
- 하단 네비게이션 5탭 재구성 (홈/타임라인/베이비페어/체중/영상) — 홈 Feature Grid과 일치
- 기존 E2E 테스트 재작성

**추가 (2026-04-03 기획 보완 2차):**

- 대시보드 하단 피드백 배너 (구글 폼 링크, 새 탭 열기)
- 베이비페어 카드 클릭 → 외부 링크 확인 팝업 → `official_url` 새 탭 열기
...
- 필터 적용 시 해당 카테고리 체크리스트가 없는 주차는 타임라인 카드째 숨김 처리 (노이즈 감소)
- **행정 준비** 카테고리 선택 시 또는 행정 준비 항목 표시 시, 상단에 안내 문구 표시:
...
커스텀 항목(타임라인/체크리스트) 삭제 시 **확인 다이얼로그** 표시:

- "이 항목을 삭제하시겠습니까?" 메시지
- [취소] / [삭제] 버튼
- 삭제는 되돌릴 수 없으므로 실수 방지 필수

### 8. 네비게이션 변경

| AS-IS | TO-BE |
|-------|-------|
| 홈 / 체크리스트 / 타임라인 / 체중 / 더보기 | 홈 / 타임라인 / 베이비페어 / 체중 / 영상 |

- 5개 탭 유지 (체크리스트 → 베이비페어로 교체, 더보기 → 영상으로 교체)
- 홈 Feature Grid(타임라인/베이비페어/체중/영상)과 네비게이션 탭이 일치하도록 구성
- **"더보기" → "영상"**: `Ellipsis` → `Video` 아이콘
- **베이비페어 탭 추가**: `Users` 아이콘, `/baby-fair` 경로

### 9. 대시보드 피드백 배너

대시보드 최하단에 **사용자 피드백 수집 배너**를 표시한다.

- 위치: 기존 "Motivational Text" 아래
- 구성: 안내 문구 + 구글 폼 텍스트 링크
- 문구: "더 나은 서비스를 위해 의견을 들려주세요"
- 링크 동작: `target="_blank"` + `rel="noopener noreferrer"` (새 탭)
- 폼 URL 관리: 환경변수 `NEXT_PUBLIC_FEEDBACK_FORM_URL`
  - 값이 비어있거나 미설정 시 배너 미노출 (피드백 수집 종료 시 대응)

### 10. 베이비페어 외부 링크 팝업

베이비페어 카드 클릭 시 **외부 이동 확인 팝업**을 표시한다.

- 트리거: 베이비페어 카드 전체 영역 클릭
- 팝업: AlertDialog (기존 DeleteConfirmDialog 패턴 활용)
  - 타이틀: "{베이비페어명}"
  - 설명: "공식 홈페이지로 이동합니다"
  - 버튼: [취소] / [이동]
- 확인 시: `window.open(official_url, '_blank', 'noopener,noreferrer')`
- `official_url` 미존재 시: 클릭 비활성 + `cursor-pointer` 제거

### 11. 대시보드 "이번 주 할 일" 보완

홈 대시보드의 "이번 주 할 일" 섹션에 **커스텀 타임라인 항목도 포함**:

- 현재: `timeline_items.json`(기본 데이터)만 표시
- 변경: `useTimelineStore.customItems`도 합산하여 필터링
...
| 11 | 네비게이션 5탭 구성 (홈/타임라인/베이비페어/체중/영상) | E2E |
| 12 | 네비게이션 라벨이 홈 Feature Grid과 일치 | E2E |
| 13 | 행정 준비 카테고리 선택 시 지자체 확인 안내 문구 표시 | E2E |
| 14 | 커스텀 항목 삭제 시 확인 다이얼로그 표시 | E2E |
| 15 | 대시보드 "이번 주 할 일"에 커스텀 타임라인 항목 포함 | E2E |
| 16 | 기존 localStorage 데이터 호환 | 수동 검증 |
| 17 | 빌드 성공 (`next build`) | CI |
| 18 | 반응형 (모바일/태블릿/데스크톱) | E2E |
| 19 | 대시보드 하단 피드백 배너 노출 + 새 탭 열기 (`NEXT_PUBLIC_FEEDBACK_FORM_URL` 설정 시) | E2E |
| 20 | 베이비페어 카드 클릭 → 확인 팝업 → 새 탭으로 `official_url` 이동 | E2E |
...
| 행정 준비 항목이 지역별로 다름 | 사용자 혼란, 잘못된 정보 전달 | 지자체 확인 필요 안내 문구 상시 표시 |
| 삭제 실수 시 복구 불가 | 커스텀 항목 유실 | 삭제 확인 다이얼로그 추가 |
