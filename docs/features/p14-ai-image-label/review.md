...
## 5.2 구현 후속 결정 (렌더 검수 단계)

- **figcaption 조건부 렌더 + 캡션 컨벤션**: 2026-05-08 결정. 운영자 실제 렌더 결과 검수 시 §5 결정의 figcaption 단독 표기(`· AI 생성`)에서 leading `·`가 캡션 텍스트 없이 허공에 떠 있는 모양이 어색하다는 피드백. 절충안 채택:
  - **figcaption은 원본 캡션이 있을 때만 렌더** (4 분기):
    - 캡션 있음 + AI 마커 있음 → `<원본 캡션> · AI 생성`
    - 캡션 있음 + AI 마커 없음 → `<원본 캡션>`
    - 캡션 없음 + AI 마커 있음 → figcaption 미렌더 (워터마크 칩 + alt 두 채널로 표시 의무 충족)
    - 캡션 없음 + AI 마커 없음 → figcaption 미렌더
  - **캡션 컨벤션은 markdown image title 슬롯**: `![alt](src "원본 캡션")`. 표준 문법 활용 + 운영자 부담 0줄. plugin이 `img.title`을 추출해 figcaption으로 옮기고 img에서 title 속성 제거(브라우저 기본 tooltip 중복 방지). sanitize는 기본 스키마가 `<img title>`을 strip하므로 `defaultSchema` spread + `attributes.img`에 `'title'` 1줄 추가하는 확장 스키마를 articles.ts에서 정의해 `rehypeSanitize(schema)`로 전달.
- 결정 영향:
  - §5 결정의 표시 형태(옵션 C — 셋 다)는 **캡션 사용 시점부터** 그대로 살아남고, 캡션 없는 글은 **칩 + alt 두 채널**로 축소. §3.1 marketer 주장의 본질("캡션 미독자 커버")은 칩이 우하단에 항상 노출되므로 유지. designer가 거부했던 "둘 다는 거부"와는 캡션 없는 시점에서만 부분적으로 일치.
  - §5.1의 alt 컨벤션은 AI 라벨 트리거 단일 source of truth로 유지. title은 캡션 텍스트 슬롯으로만 사용해 책임 분리.
  - 발행 글 2건은 캡션 없음 → 칩 + alt만 노출. 마이그레이션 추가 작업 0.
- 잔재:
  - 운영자 시각 점검에서 캡션 미독자 커버 약화가 문제로 부각되면 figcaption 항상 렌더(`AI 생성 이미지` 단독 표기 등)로 재논의.
  - title 슬롯의 또 다른 표준 용도(접근성 보조 설명)와 충돌 가능 — 본 사이트는 캡션 용도로 1:1 점유. 향후 다른 용도 필요 시 frontmatter 캡션 맵으로 이전 재논의.
