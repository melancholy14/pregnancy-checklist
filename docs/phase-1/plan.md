...
#### LocalStorage 용량 분석 및 전략

**왜 4개 store가 각각 독립적인 localStorage key를 사용하는가:**

Zustand persist는 store 단위로 직렬화/역직렬화한다.
하나의 key에 합치면 어느 store든 변경 시 전체를 직렬화해야 하므로
성능 저하와 데이터 충돌 위험이 생긴다.
독립 key 방식은 각 store가 자기 데이터만 read/write하므로 안전하다.

**Store별 용량 추정:**

| Store | Key | 예상 최대 용량 | 근거 |
| ----- | --- | -------------- | ---- |
| `due-date-storage` | 출산 예정일 | ~50 bytes | string 1개 |
| `checklist-storage` | 체크 ID + 커스텀 항목 | ~15 KB | checkedIds 120개(~2KB) + customItems 최대 50개(~13KB) |
| `timeline-storage` | 커스텀 타임라인 | ~10 KB | customItems 최대 40개(주차별 1개) |
| `weight-storage` | 체중 기록 | ~20 KB | 40주 × 매주 기록 시 ~40개 entry |
| **합계** | | **~45 KB** | |

**브라우저 localStorage 한도**: 5~10 MB (브라우저별 상이, 최소 5MB)
→ 예상 사용량은 한도의 **1% 미만**이므로 용량 초과 위험 없음.

**스키마 버전 관리:**

현재 persist에 `version` 옵션을 사용하지 않는다.
Phase 1에서는 스키마가 안정적이므로 불필요하지만,
Phase 2 이후 스키마 변경 시 `version` + `migrate` 옵션 추가 필요:
...
localStorage 접근 실패(시크릿 모드, 용량 초과) 시
Zustand persist는 자동으로 메모리 전용 모드로 폴백한다.
별도 에러 핸들링 불필요.
