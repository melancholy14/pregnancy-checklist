...
- 그 인스타 계정이 **Facebook 페이지에 연결**돼 있어야 함 (Meta Business Suite는 페이지 단위로 관리).
- [business.facebook.com](https://business.facebook.com) 로그인 시 해당 페이지 관리자 권한 필요.

**게시물당 예약 절차**:
1. `tools/cardnews/out/<slug>/` 폴더에서 `01.png ~ 0N.png` 순서 확인, `INDEX.txt`에서 캡션·해시태그 복사.
2. Meta Business Suite → 좌측 메뉴 **"플래너(Planner)"** → **"게시물 만들기"** → **Instagram 게시물** 선택.
3. **"캐러셀 추가"**로 PNG를 **번호 순서 그대로**(01→마지막) 업로드 — 순서가 카드 스토리라인이라 뒤바뀌면 안 됨.
4. 캡션 입력창에 `INDEX.txt`의 caption 텍스트 붙여넣기, 이어서 hashtags 붙여넣기(캡션 맨 끝 또는 첫 댓글로 분리해도 무방).
5. 우측 상단 **"예약"** 선택 → 스케줄 표의 발행일(매주 목요일) + 원하는 시간대 지정.
6. **"예약 완료"** 클릭. 플래너 캘린더에서 예약된 게시물로 확인 가능 — 발행 전까지 언제든 수정·삭제 가능.
7. #7~#15를 한 번에 몰아서 예약해두면, 이후엔 플래너 캘린더만 가끔 확인하면 됨 (게시물 자체는 지정 시각에 자동 발행).

**주의**:
- 예약된 게시물도 발행 전이면 수정 가능하니, 발행 하루 전쯤 캡션에 오타·링크 문제 없는지 한 번 훑어보는 걸 권장.
- 바이오 링크(프로필 URL)는 예약 발행과 별개로 항상 최신 상태(현재 발행 글 또는 링크 허브)로 유지해야 CTA가 안 깨짐.
- Meta 정책상 예약은 최대 75일 전까지 가능 — #7~#15 전체(약 2개월 분량)는 한 번에 다 넣을 수 있음.
