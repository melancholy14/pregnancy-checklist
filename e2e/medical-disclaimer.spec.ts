...
test.describe("의료 디스클레이머 + reviewed_by 정리 (Step 0e)", () => {
  ...
  test.describe("Happy Path", () => {
    ...
    test("산후조리원 아티클에 소아청소년과 전문의 안내가 노출된다", async ({ page }) => {
      // 무엇을: 산후·신생아 주제 아티클에 산부인과·소아청소년과 전문의 안내 확인
      // 왜: 글 주제와 면책 대상이 일치해야 신뢰도 향상 (feedback_disclaimer_by_topic 룰)
      await page.goto("/articles/postpartum-care-center-guide");

      const disclaimer = page.getByText(/소아청소년과 전문의와 상담/);
      await expect(disclaimer).toBeVisible();
    });
  ...
  test.describe("반응형 (Mobile 375px)", () => {
    ...
    test("모바일에서 면책 문구가 정상 표시된다", async ({ page }) => {
